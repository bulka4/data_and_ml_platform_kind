...
#   - iceberg-spark-runtime - For uisng Iceberg (so Spark can create Iceberg tables)

# Save them in the /opt/spark/jars/ folder because that folder is included in the Spark's classpath so that will allow Spark to use them.
...
RUN wget https://repo1.maven.org/maven2/org/apache/hadoop/hadoop-azure/3.3.4/hadoop-azure-3.3.4.jar -P /opt/spark/jars/ \
    ...
    && wget https://repo1.maven.org/maven2/org/apache/iceberg/iceberg-spark-runtime-3.5_2.12/1.9.2/iceberg-spark-runtime-3.5_2.12-1.9.2.jar -P /opt/spark/jars/